...
public class MainMFD : MonoBehaviour
{
    ...
    public RectTransform mainMFDPanel;
    public Image[] mfdGUI;
    //
    ...
    public float idleTemperature = 35f;//, criticalTemp = 90f;
    ...
    //////[Tooltip("Time in Minutes to consume 100% Fuel at Máx Engine RPM")]
    //////public float maxfuelFlow = 1.00f;
    //////[Tooltip("Time in Minutes to consume 100% Fuel at Iddle Engine RPM")]
    ...
    void Awake()
    {
        ...
        if (mainMFDPanel == null) mainMFDPanel = GetComponent<RectTransform>();
    ...
    //////////    {
    //////////        current = null;
    //////////        foreach (MainMFD mainMFD in FindObjectsOfType<MainMFD>()) if (mainMFD != this && mainMFD.gameObject.activeInHierarchy) { current = mainMFD; break; }
    //////////    }
    //////////}
    //
    void OnEnable()
    ...
    {
        SndPlayer.playClick();
        mainMFDPanel.gameObject.SetActive(!mainMFDPanel.gameObject.activeSelf);


        if (!mainMFDPanel.gameObject.activeSelf)
        {
    ...
    public void setTextOutline(bool active = true) { foreach (Outline outLine in mainMFDPanel.GetComponentsInChildren<Outline>(true)) outLine.enabled = active; }
    public void setGaugesColor(int index = -1)
    {
        ColorImg[] colorImg = mainMFDPanel.GetComponentsInChildren<ColorImg>(true);
    ...
    {
        ColorTxt[] colorTxt = mainMFDPanel.GetComponentsInChildren<ColorTxt>(true);
    ...
    {
        ToogleImg[] masks = mainMFDPanel.GetComponentsInChildren<ToogleImg>(true);
    ...
    void FixedUpdate() //Update()
    {
        ...
        if (useFuel)
        {
            ...
            //Consumption in % per Minute
            //if (fuelFlowFillUI != null) fuelFlowFillUI.fillAmount = (0.6f * engineReNormalized * fuelFlow / (maxfuelFlow * Time.fixedDeltaTime));
            //if (fuelFlowTxt != null) fuelFlowTxt.text = (fuelAmplitude * fuelFlow * fuelFlowAmplitude).ToString("##0.0");//.ToString("0.0").PadLeft(4);  //.ToString("##0");     
            //
            //Consumption in Minutes for 100% Fuel
